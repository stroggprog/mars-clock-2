...
What was the problem? The DietPi update script which installed the new kernel didn't preserve the `dtparam` setting for the screen in the startup config file (the DietPi update script warns this will happen). Running the script `MHS35-show 180` rebuilt the drivers (in case the build changed with the new kernel) and added the `dtparam` setting to the config file.
